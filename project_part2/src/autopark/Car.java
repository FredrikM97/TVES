...
public class Car extends ACar {
	...
	public StatusWrapper<Boolean> park() {
		...
		// Action
		while (this.getPosition() < this.getStreetLength() - 5) {
		...
		this.setParkingSpace(0);

		// TODO replace all .setParkingSpace(5) in CarTest with setBestParkLength
		if (this.getBestParkSpace() >= 5) {

			// Move till distance is 0 between best spot and current position
			while (this.getPosition() - this.getBestParkPosition() > 0) {
				if (!(moveBackward().getStatus().equals(StatusWrapper.OK)))
					return new StatusWrapper<>(false, autopark.StatusWrapper.UNEXPECTED_STATE, "No working actuator");
	...
	public StatusWrapper<Boolean> unPark() {
		...
		// Action
		if (!this.getMoveCar().activate(5, 315)) {
	...
	private Optional<Double> readAndFilter(Sensor sens) {
		...
		final double std = Math.sqrt(reads.stream()
			.map(read -> (read - mean) * (read - mean))
			.reduce(0.0, Double::sum)/(reads.size()-1));
